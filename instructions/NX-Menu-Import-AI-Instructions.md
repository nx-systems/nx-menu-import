# NX Menu Import — AI Instruction Pack

**Version 2.0 · For NX Restaurant dealers**

Derived from the NX help center articles *Importing Menu Items Using a Template* and *Understanding the Menu Item Import CSV Template*.

## How to use this

1. Open any AI assistant (Claude, ChatGPT, Gemini, Copilot).
2. Paste this entire document into the chat.
3. Attach or paste the venue's menu (PDF, photos, or typed text; a website URL only if your AI tool can open links — otherwise paste the page text or screenshots).
4. Say: *"Follow the NX Menu Import instructions. Here is the menu."*
5. Answer the assistant's checkpoint questions, then take the CSV it produces and import it at **Configuration > Import > NX Menu Item CSV**.

Everything the assistant needs is in this document. It does not need to look anything up. If you also hand it the template spreadsheet, it should take only the header row from it and ignore the example rows.
...
**`POS Name` must therefore be unique across the entire file**, not just within a category. Two rows sharing a `POS Name` are one record to the importer, and the second overwrites the first. Where two different things would naturally share a name, make the names distinct (see Section 7).
